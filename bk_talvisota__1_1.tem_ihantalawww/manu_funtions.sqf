...
{
  params ["_markersNamesArray",["_ammo", "Sh_82mm_AMOS"],["_minSleep", 2],["_maxSleep", 2],["_helpAI", nil],["_message", ""],["_flag", nil]];
                                        //Sh_155mm_AMOS
  if (isNil "_ammo") then {
    _ammo = "Sh_82mm_AMOS";
  };
  ...
  while { _f == true } do {
    ...
    if (!isNil "_helpAI") then {
      {
        _x setUnitPos "MIDDLE";
        //_x disableAI "PATH"; <-- ojeli
        _x allowDammage false;
      } forEach units group _helpAI;
      group _helpAI setCombatMode "BLUE";
    };
    ...
    if (!isNil "_message") then {
      _message call SAC_fnc_MPtitleText;
    };

    if (!isNil "_helpAI") then {
      //sleep (_maxSleep + 10);
      uiSleep (_maxSleep + 10);
      {
        _x setUnitPos "UP";
        //_x disableAI "PATH"; <-- ojeli
        _x allowDammage true;
      } forEach units group _helpAI;
      group _helpAI setCombatMode "RED";
    };
