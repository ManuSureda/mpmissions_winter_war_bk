//titleText ["Combat Operations", "BLACK FADED", 200];
...
// Definir el nivel de detalle del terreno y vision de comienzo
setTerrainGrid 35;
...
[] call compile preprocessFileLineNumbers "manu_funtions.sqf";

[] call compile preprocessFileLineNumbers "SAC_GEAR\main.sqf";

[] call compile preprocessFileLineNumbers "MISC\def_gears.sqf";
[] call compile preprocessFileLineNumbers "MISC\fnc_dressAs.sqf";
[] call compile preprocessFileLineNumbers "MISC\fnc_equipments.sqf";
[] call compile preprocessFileLineNumbers "MISC\fnc_uniforms.sqf";
[] call compile preprocessFileLineNumbers "MISC\fnc_weapons.sqf";

[] call compile preprocessFileLineNumbers "MISC\manu_aux.sqf";
...
if (!isNull player) then {
	...
	[] spawn {
		...
		while {true} do {
			...
			sleep (2*60);
			player removeItems "MineDetector";
			// player removeItems "UNS_TrapKit";
			// player removeItems "LIB_ToolKit";
			// player unlinkItem "ItemGPS";
			// player removeItems "ACE_DAGR";
			// player removeItems "ACE_microDAGR";
			// player removeItems "ItemcTab"; //Commander Tablet
			// player unlinkItem "ItemcTab"; //Commander Tablet
			// player removeItems "ItemAndroid"; //Android based Blue Force Tracker
			// player unlinkItem "ItemAndroid"; //Android based Blue Force Tracker
			// player removeItems "ItemMicroDAGR"; //MicroDAGR GPS
			// player unlinkItem "ItemMicroDAGR"; //MicroDAGR GPS
	...
	if (!isNil "TIC_WEST") then {"Error: SAC_COP - Falta borrar definición de TIC_WEST." call SAC_fnc_MPhintC};
	if (!isNil "TIC_EAST") then {"Error: SAC_COP - Falta borrar definición de TIC_EAST." call SAC_fnc_MPhintC};
	if (!isNil "REI_WEST") then {"Error: SAC_COP - Falta borrar definición de REI_WEST." call SAC_fnc_MPhintC};
	if (!isNil "REI_EAST") then {"Error: SAC_COP - Falta borrar definición de REI_EAST." call SAC_fnc_MPhintC};
	...
	player addRating 99999; //para evitar que matar civiles convierta al jugador en renegado
...
if (isServer) then {
	...
	if ((!isNil "TAC_O_REGULAR") && {TAC_O_REGULAR}) then {
		...
		SAC_TAC_transportVehClasses = if (isNil "SAC_COP_INTERCEPT") then {SAC_UDS_O_unarmedTransport} else {SAC_UDS_eastInterceptionRegular};
