...
SAC_GEAR_applyLoadoutByClass = {
	...
	switch (typeOf _unit) do {
		...
		case "LOP_SYR_Infantry_AT_Asst":    {[_unit, SAC_GEAR_O_G_loadoutProfile, "AT_A"] call SAC_GEAR_applyLoadout};
		case "LOP_SYR_Infantry_Engineer":   {[_unit, SAC_GEAR_O_G_loadoutProfile, "ENG"] call SAC_GEAR_applyLoadout};
		case "LOP_SYR_Infantry_MG":         {[_unit, SAC_GEAR_O_G_loadoutProfile, "MG"] call SAC_GEAR_applyLoadout};
		case "LOP_SYR_Infantry_MG_Asst":    {[_unit, SAC_GEAR_O_G_loadoutProfile, "MG_A"] call SAC_GEAR_applyLoadout};
		case "LOP_SYR_Infantry_Rifleman_2": {[_unit, SAC_GEAR_O_G_loadoutProfile, "AR"] call SAC_GEAR_applyLoadout};
		...
		// ZOMBIES_loadout
		case "Zombie_G_Crawler_Civ":  {[_unit, SAC_GEAR_O_loadoutProfile, "ZMB"] call SAC_GEAR_applyLoadout};
		case "Zombie_G_Shambler_Civ": {[_unit, SAC_GEAR_O_loadoutProfile, "ZMB"] call SAC_GEAR_applyLoadout};
		case "Zombie_G_RA_Civ":       {[_unit, SAC_GEAR_O_loadoutProfile, "ZMB"] call SAC_GEAR_applyLoadout};
		case "Zombie_G_RC_Civ":       {[_unit, SAC_GEAR_O_loadoutProfile, "ZMB"] call SAC_GEAR_applyLoadout};
		case "Zombie_G_Walker_Civ":   {[_unit, SAC_GEAR_O_loadoutProfile, "ZMB"] call SAC_GEAR_applyLoadout};
		...
		// sov_w_39
		case "O_NORTH_SOV_W_39_Officer_Cpt"             : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_N_W_39_Officer"               : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_W_39_Rifleman_SSGT"           : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_W_39_Rifleman_SGT"            : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_N_W_39_Rifleman_SGT"          : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_W_39_Submachinegunner"        : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_N_W_39_Submachinegunner_38"   : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_W_39_Machinegunner"           : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_W_39_Machinegunner_Assistant" : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_W_39_Rifleman"                : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_R_W_39_Rifleman"              : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_N_W_39_Rifleman"              : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_W_39_Sniper"                  : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_W_39_Medic"                   : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_N_W_39_Medic"                 : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
		case "O_NORTH_SOV_W_39_Engineer"                : {[_unit, SAC_GEAR_O_G_loadoutProfile, "ADD"] call SAC_GEAR_applyLoadout};
...
*/		
		...
		case "SYRIAN_ARMY_ALTIS_loadout": { 
			...
			// uniform
			if (_role in ["PILOT", "CREW", "SL", "TL"]) then {
				if (_role in ["CREW"]) then { _unit forceAddUniform "U_I_E_Uniform_01_sweater_F"; };
				if (_role in ["PILOT"]) then { _unit forceAddUniform "U_I_E_Uniform_01_coveralls_F"; };
				if (_role in ["SL","TL"]) then { _unit forceAddUniform "U_I_E_Uniform_01_F"; };
			} else {
				_unit forceAddUniform selectRandom [
			...
			// backpack
			if (_role in ["MED", "MG", "MG_A", "AR", "ENG", "MM", "AT", "AT_A"]) then {
				if (_role in ["MED"]) then {
					_unit addBackpack "rhs_medic_bag";
				};
				if (_role in ["MG", "MG_A", "AR"]) then {
					_unit addBackpack selectRandom [
						"B_FieldPack_green_F",
						"B_Kitbag_rgr"
					];
			...
			//helmet
			if (_role in ["SL", "CREW", "PILOT"]) then {
				if (_role in ["SL"]) then { _unit addHeadgear "H_MilCap_eaf"; };
				if (_role in ["CREW"]) then { _unit addHeadgear "H_Tank_eaf_F"; };
				if (_role in ["PILOT"]) then { _unit addHeadgear "H_PilotHelmetHeli_I_E"; };
			} else {
				_unit addHeadgear selectRandom ["H_Booniehat_eaf", "rhssaf_beret_green"];
			};

			if (random 1 < 0.5) then {
				_unit linkItem "WBK_ShoulderLampItem_Weak";
			};
		...
		case "ZOMBIES_loadout": { 
			...
			switch (_aux) do {
				case "army": { 
					...
					if (random 1 < 0.3) then { _unit linkItem "WBK_ShoulderLampItem_Weak"; };
				};
				case "civ": { 
					_unit forceAddUniform selectRandom [
						"U_Marshal",
...
SAC_GEAR_fnc_manu_giveAK74MR_GP25 = {
	...
	[_unit, "rhs_30Rnd_545x39_7N10_AK", 4] call SAC_fnc_addMagazines;
	[_unit, "rhs_30Rnd_545x39_AK_plum_green", 4] call SAC_fnc_addMagazines;
	[_unit, "rhs_VOG25", 5] call SAC_fnc_addMagazines;
...
SAC_GEAR_fnc_manu_giveSVD_camo = {
	...
	[_unit, "rhs_10Rnd_762x54mmR_7N14", 10] call SAC_fnc_addMagazines;
...
SAC_GEAR_fnc_manu_giveRPK74M = {
	...
	[_unit, "rhs_45Rnd_545X39_AK_Green", 10] call SAC_fnc_addMagazines;
...
SAC_GEAR_fnc_giveWeapons = {
	...
	*/
		...
		case "Syrian_army_altis_wapons": {
			...
			if (_role in ["MED"]) then {
				[_unit,_role] call SAC_GEAR_fnc_manu_giveAK105_Zenetico01;
	 			if (random 1 < 0.3) then { [_unit, "SmokeShell", 1] call SAC_fnc_addMagazines; };
			};
			if (_role in ["GL"]) then {
				[_unit,_role] call SAC_GEAR_fnc_manu_giveAK74MR_GP25;	
				[_unit, "rhs_mag_rgd5", 1] call SAC_fnc_addMagazines;
			};
			if (_role in ["AT"]) then {
				[_unit] call SAC_GEAR_fnc_manu_giveAK105_Zenetico01;
				[_unit] call SAC_GEAR_fnc_manu_giveRPG7_F;	
				if (random 1 < 0.3) then { [_unit, "rhs_mag_rgd5", 1] call SAC_fnc_addMagazines; };
			};
			if (_role in ["AT_A"]) then {
				[_unit] call SAC_GEAR_fnc_manu_giveAK105_Zenetico01;
				[_unit, "RPG7_F", 3] call SAC_fnc_addMagazines;
				if (random 1 < 0.3) then { [_unit, "rhs_mag_rgd5", 1] call SAC_fnc_addMagazines; };
			};
			...
			if (_role in ["MG_A"]) then {
				[_unit] call SAC_GEAR_fnc_manu_giveAK105_Zenetico01;
				[_unit, "75rnd_762x39_AK12_Mag_Tracer_F", 2] call SAC_fnc_addMagazines;
				if (random 1 < 0.3) then { [_unit, "rhs_mag_rgd5", 1] call SAC_fnc_addMagazines; };
			};
			if (_role in ["AR"]) then {
